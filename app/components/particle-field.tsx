...
export function ParticleField() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      g.addColorStop(0, "color-mix(in oklch, var(--color-background) 70%, var(--color-primary) 30%)")
      g.addColorStop(1, "color-mix(in oklch, var(--color-background) 70%, var(--color-accent) 30%)")
      ctx.fillStyle = g
      ...
      for (const s of stars) {
        ...
        ctx.fillStyle = `oklch(0.92 ${0.03 + s.r * 0.02} ${260 + s.r * 30} / ${0.25 + 0.55 * alpha})`
        ctx.fill()
      }
